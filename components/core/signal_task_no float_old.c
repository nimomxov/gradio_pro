...
/* مؤشر لـ SensitivityManager المُهيَّأ في app_main */
extern SensitivityManager_t g_sens_mgr;

/* =========================================================================
 * CALIBRATION RETRY CONFIGURATION
 * ========================================================================= */
#define CALIB_MAX_RETRIES        3u      /* أقصى عدد محاولات إعادة المعايرة     */
#define CALIB_RETRY_DELAY_MS     2000u   /* delay بين المحاولات (ms)             */
#define CALIB_RETRY_MAX_DELAY_MS 10000u  /* أقصى delay (exponential backoff)     */
...
static struct {
    ...
    uint32_t          calib_retry_delay_ms;   /* يتضاعف مع كل محاولة فاشلة */
    bool              calib_waiting;           /* true = تجاهل ADC حتى ينتهي الـ delay */
        ...
        switch (msg.event) {
            ...
            case SYS_EVT_TOUCH_CALIB:
                if (s_sig.state == ST_ACTIVE || s_sig.state == ST_IDLE) {
                    s_sig.calib_waiting        = false;   /* إلغاء أي retry wait قديم */
            ...
            /* ═══════════════════════════════════════════════════════════
             * Sensitivity Change — من UI أو من Auto Engine
             *
             * Loop-prevention: إذا كان المود المطلوب = المود الفعلي
             * (effective_mode)، نتجاوز apply_mode لقطع الحلقة التي
             * قد تنشأ عندما apply_mode ترسل SYS_EVT_SENS_CHANGE مجدداً.
             * ═══════════════════════════════════════════════════════════ */
            case SYS_EVT_SENS_CHANGE:
                ...
                /* دائماً حدِّث الـ DSP بالمود المطلوب */
            ...
            case SYS_EVT_SLEEP_REQ:
                if (s_sig.state == ST_CALIBRATING) {
                    ...
                    /* إجبار الإلغاء في الدورة القادمة */
...
static void process_calibrating(const AdcSample_t *sample)
{
     ...
     * إذا كنا في وضع الانتظار، تحقق هل انتهى الوقت.
     * إذا لم ينتهِ → أهمل العينة وعُد. الـ task لا يتجمد.
     * إذا انتهى   → أعد تشغيل calib_engine الآن.
     * ─────────────────────────────────────────────────────────── */
    ...
    switch (status) {
        ...
        case CALIB_STATUS_ERROR:
            ...
            if (s_sig.calib_retry_count <= CALIB_MAX_RETRIES) {
                ...
                /* Non-blocking delay: فعّل وضع الانتظار.
                 * process_calibrating ستتجاهل العينات حتى ينتهي الوقت.
                 * handle_events() تستمر في العمل → UI لا يتجمد. */
                s_sig.calib_waiting        = true;
                s_sig.calib_retry_after_tick = xTaskGetTickCount()
                                             + pdMS_TO_TICKS(s_sig.calib_retry_delay_ms);

                /* Exponential backoff: ضاعف الـ delay للمحاولة القادمة */
                ...
                /* calib_engine_start يُستدعى بعد انتهاء الـ delay في process_calibrating */
            } else {
                ESP_LOGE(TAG, "Calibration failed after %u retries — reverting to IDLE",
                         CALIB_MAX_RETRIES);
                s_sig.calib_retry_count    = 0;
                s_sig.calib_retry_delay_ms = CALIB_RETRY_DELAY_MS;
                s_sig.calib_waiting        = false;   /* إلغاء الانتظار */
                s_sig.state                = ST_IDLE;
            }
            break;
...
static void process_active(const AdcSample_t *sample)
{
    ...
    if (valid) {
         ...
         * مُجمَّد أثناء Boost Mode:
         *   في Boost (256 samples) الـ noise ينخفض مصطنعاً (~18dB).
         *   لو تركنا Auto يعمل، سيرى noise منخفض جداً ويرفع الـ sensitivity
         *   لـ VERY_HIGH رغم أن المستخدم يريد ثبات المود في الأعماق.
         * ─────────────────────────────────────────────────────────── */
        if (!s_sig.boost_mode) {
...
static void apply_pending_filter_requests(void)
{
    ...
    if (s_boost_valid) {
        ...
        /* تطبيق الـ ADC oversampling فوراً عند تغيير Boost */
