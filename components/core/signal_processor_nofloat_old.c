...
{
    float signal = signal_in;   /* baseline-subtracted OR spatial gradient */

/* -- Compute range ? used by NORMALIZED/ENHANCED only --------------- */
    float bt_range;
    if (spatial_gradient) {
    ...
    /* -- TRUE RAW MODE ? for OKM Visualizer3D --------------------------
     *
     * DESIGN PRINCIPLE: Visualizer3D performs its own adaptive normalization.
     *   After the first 5?10 scan steps, it builds a soil reference model
     *   and assigns colors relative to it (green=soil, yellow/red=metal,
     *   blue=void). Re-normalizes again at the end of each scan session.
     *   Feeding it pre-compressed data DESTROYS this capability.
     *
     * WHAT WE SEND: signal + 512
     *   signal = filtered_Kalman - calibrated_baseline
     *          = pure physical anomaly in absolute LSB units, zero-centered
     *          = 0 LSB  ? quiet soil   ? maps to 512 (Visualizer center)
     *          = +50 LSB ? metal       ? maps to 562
     *          = -50 LSB ? void        ? maps to 462
     *
     * WHY NOT raw_differential + 512:
     *   raw_differential ? baseline_DC (?8000?20000 LSB from Earth field
     *   + sensor mismatch). Adding 512 clips everything to 1024 immediately.
     *   The calibrated baseline subtraction IS necessary ? it removes the
     *   sensor DC offset, not the target signal. Visualizer3D cannot do this
     *   itself because it doesn't know the sensor's physical DC offset.
     *
     * WHY NOT bt_range compression (old code):
     *   bt_range = k ? noise_floor ? different for every soil type.
     *   A target producing +30 LSB deviation in pristine soil (noise=1)
     *   maps to 800+ (bright red). Same target in mineral soil (noise=10)
     *   maps to 560 (barely visible). Visualizer3D sees inconsistent data
     *   across different field conditions ? incorrect depth estimation.
     *
     * SPATIAL GRADIENT + RAW: bypass spatial, use direct signal.
     *   Visualizer3D expects the physical anomaly signal, not its spatial
     *   derivative. Derivatives lose absolute magnitude ? Visualizer3D
     *   cannot distinguish depth (amplitude-based in its algorithm).
     *
     * RESULT: Visualizer3D receives a faithful, linear representation of
     *   the physical magnetic anomaly. It normalizes per-scan as designed.
     *   Same target at same depth = same color regardless of soil type.
     * ------------------------------------------------------------------- */
    if (sp->bt_mode == BT_MODE_RAW) {
        /* -- Uncalibrated RAW: use raw differential directly ------------
         * ChatGPT audit: Visualizer3D depends on signal shape and relative
         * amplitude. Any filtering (Kalman, EMA, MA) reduces high-frequency
         * content and softens target edges ? both needed for accurate depth
         * estimation in the Visualizer algorithm.
         *
         * When calibrated: signal is already clean (Kalman + baseline sub).
         *   ? Use signal as-is (filtered - calibrated_baseline + 512).
         *
         * When uncalibrated: signal = filtered - initial_mean_warmup.
         *   The Kalman has been applied but initial_mean substitutes for
         *   the calibrated baseline. This is acceptable for calibrated mode.
         *   However, for maximum Visualizer3D accuracy, we bypass filtering
         *   and use (raw_differential - initial_mean + 512) directly.
         *   ? Raw shape preserved ? better depth estimation.
         *
         * Note: raw_diff here comes from signal_in which is the full
         *   baseline-subtracted signal passed from sp_process. When
         *   uncalibrated, pass the raw value via sp->uncalib_initial_mean
         *   stored from warmup. The caller (sp_process) passes `signal`
         *   which = filtered - baseline_fixed. For uncalibrated,
         *   baseline_fixed = initial_mean ? signal = filtered - initial_mean.
         *   Close to raw - initial_mean, but filtered. The true raw path
         *   is captured below using sp->prev_raw (last raw sample).
         * --------------------------------------------------------------- */
        float raw_centred;
        if (!sp->calibrated && sp->uncalib_warmup_done) {
            /* Use prev_raw (last decimated sample before any filtering).
             * Note: sp->prev_raw holds the raw value from the PREVIOUS call
             * because prev_raw is updated at the END of sp_process after
             * pipeline_bt returns. In practice this is one sample old (~50ms
             * at 20Hz) ? negligible latency for archaeology scan speeds.
             * The 2-sample average was removed here because pipeline_bt does
             * not have access to the current AdcSample_t (by design ? it is
             * a read-only pipeline helper, not a sample consumer). */
    ...
    /* -- NORMALIZED / ENHANCED: apply slow DC drift removal -----------
     * Removes very slow baseline drift across long scan distances.
     * Time constant ~100s @ 20Hz ? targets pass through unaffected.
     * bt_drift lives in the struct (Core0-only access, no race condition).
     * Skip drift removal in spatial mode ? gradient is already DC-free.
     * ------------------------------------------------------------------- */
    float signal_hp;
    if (spatial_gradient) {
        signal_hp = signal;   /* gradient is already zero-mean ? no drift needed */
    } else {
    ...
    /* -- ENHANCED: tighter range ? better contrast for weak targets ---
     * Amplifies visual contrast without distorting signal shape.
     * Trade-off: strong targets clip to 0/1024 (still detectable, just saturated).
     * Ideal for fine jewelry, small coins, shallow gold deposits.
     * ------------------------------------------------------------------- */
    float map_min = bt_min;
    float map_max = bt_max;
    if (sp->bt_mode == BT_MODE_ENHANCED) {
        map_min *= 0.7f;    /* tighter than 0.8 ? stronger visual contrast */
        map_max *= 0.7f;
    }

    float norm = (signal_hp - map_min) / (map_max - map_min);
    norm = fminf(fmaxf(norm, 0.0f), 1.0f);
    return (uint16_t)(norm * 1024.0f);
}
